...
/***********************************************密码修改页面*******************************************************/
    ...
    public function infoSave(){
        ...
        /**
         * ADD by Minas 2014-04-22
         **/
        $profile = $tbUserProfile->where("uid = '$uid'")->find();
...
/****************************************获取用户信息*******************************************/
 ...
 /*****************************根据获得的pub_id和is_complete来修改该信息的出售状态*****************/
    ...
    public function pubComplete(){
        ...
        /** add by minas 2014-04-22 
        ** get pub.stock and decrease it.
        **/
       /* $pub_data = $tbPub->where($condition)->find();
        $data = array( 'is_complete'=> $is_complete,'stock' => ($pub_data['stock']-1) );*/

        /**
        ** end modified by minas 2014-03-22
        **/
    ...
    public function addressManage()
    {
        ...
        $profile = $userProfile->where("uid = '$uid'")->find();
        $profile = json_decode($profile['data'],true);

        if(isset($profile['addr']) && !empty($profile['addr'])) {
            $this->assign('addrs',$profile['addr']);
        }
        else 
            $this->assign('adrrs','暂时还没有添加地址');
    ...
    public function saveAddr()
    {
        ...
        if(isset($profile['addr'])){
            $count = count($profile['addr']);
            $profile['addr'][$count] =  $addr;
        } else {
            $profile['addr'] = array(
                0 => $addr);
        }
        $userProfile->where("uid = '$uid'")->save(array("data" =>  json_encode($profile)));

        $returnData["isAdd"] = true;
        $returnData["msg"] = "地址已保存";
        $this->ajaxReturn($returnData);
    }

    // add by minas 2014-05-03
